...
# Query the actual hostname being advertised by mDNS
# Wait for otbr-agent to startup if needed
sleep 2
if command -v ot-ctl >/dev/null 2>&1; then
    MDNS_HOSTNAME=$(ot-ctl mdns localhostname 2>/dev/null \
        | grep -v "Done" \
        | grep -v "^Error" \
        | tr -d '[:space:]' \
    ...
    if [ -n "$MDNS_HOSTNAME" ]; then
        echo "Detected OpenThread mDNS hostname: ${MDNS_HOSTNAME}.local"
    else
        MDNS_HOSTNAME=$(hostname)
        echo "Warning: Could not get mDNS hostname, using system hostname: ${MDNS_HOSTNAME}.local"
    fi
else
    MDNS_HOSTNAME=$(hostname)
    echo "Warning: ot-ctl not found, using system hostname: ${MDNS_HOSTNAME}.local"
fi
...
http:
  routers:
    ...
    wellknown-router:
      rule: "(Host(\`${HOSTNAME}.local\`) || Host(\`localhost\`)) && PathPrefix(\`/.well-known/thread\`)"
      entryPoints:
        - websecure
      service: otbr-auth
      tls:
    ...
    auth-router:
      rule: "(Host(\`${HOSTNAME}.local\`) || Host(\`localhost\`)) && (Path(\`/api/auth/password\`) || Path(\`/api/auth/token\`) || PathPrefix(\`/api/auth/certs\`) || Path(\`/api/auth/enroll\`))"
      entryPoints:
        - websecure
      service: otbr-auth
      tls:
    ...
    api-router-operational:
      rule: "(Host(\`${HOSTNAME}.local\`) || Host(\`localhost\`)) && PathPrefix(\`/api\`)"
      entryPoints:
        - websecure
      service: otbr-agent
      tls:
    ...
    web-router:
      rule: "(Host(\`${HOSTNAME}.local\`) || Host(\`localhost\`)) && PathPrefix(\`/\`)"
      entryPoints:
        - websecure
      service: otbr-web
      tls:
...
if [ -d "$PROJECT_ROOT/traefik/wsgi_app" ]; then
    # Copy wsgi_app to data directory for system installation
...
if curl -k --connect-timeout 5 "https://localhost/.well-known/thread" >/dev/null 2>&1; then
    ...
    echo ""
    echo "Default credentials: admin/thread"
else
    echo "⚠ Warning: Cannot reach Traefik HTTPS endpoint"
    echo ""
