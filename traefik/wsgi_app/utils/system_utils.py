...
def get_hostname():
    ...
    try:
        # Try reading /etc/hostname first
        with open('/etc/hostname', 'r') as f:
            hostname = f.read().strip()
    except (FileNotFoundError, IOError):
        # Fallback to Python's socket module
...
def switch_config_files(mode, traefik_dir):
    ...
    """
    traefik_dir = Path(traefik_dir) if not isinstance(traefik_dir, Path) else traefik_dir
    ...
    try:
        ...
        config_content = template_content.replace('otbr-hostname.local', f'{hostname}.local')
...
def should_switch_to_mtls(client_authz_file):
    ...
    """
    client_authz_file = Path(client_authz_file) if not isinstance(client_authz_file, Path) else client_authz_file
    ...
    try:
        ...
        if not clients:
            logger.debug("No registered clients found, staying in basic auth mode")
            return False
        ...
        for client_id, client_info in clients.items():
            ...
            if role == 'admin' and isinstance(sans, list) and len(sans) > 0:
                logger.info(f"Found admin client (ID: {client_id[:16]}...) with {len(sans)} SAN(s), will switch to mTLS")
                has_admin = True
                break
        
        if not has_admin:
            logger.debug("No admin client with valid SANs found, staying in basic auth mode")
            return False
